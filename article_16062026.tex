...
% Abstract
%=================================================================
\abstract{\textcolor{blue}{Soil pollution is a critical global environmental challenge arising from industrialization, intensive agriculture, urban expansion, mining, and the application of fertilizers, pesticides, and synthetic chemicals. This study presents and evaluates seven annotated Python workflows for soil pollution assessment across five major contaminant groups: heavy metals, pesticides, microplastics, per- and polyfluoroalkyl substances (PFAS), and excess macronutrients. Drawing on evidence from more than 100 peer-reviewed studies, the workflows demonstrate a reproducible, end-to-end analytical pipeline integrating Random Forest regression, XGBoost gradient boosting, convolutional neural networks (CNNs), LSTM-based time-series forecasting, Principal Component Analysis (PCA), Monte Carlo uncertainty simulation, and geospatial hotspot mapping via GeoPandas. Advanced spectroscopic approaches---Fourier transform infrared spectroscopy (FTIR), X-ray photoelectron spectroscopy (XPS), hyperspectral imaging, and synchrotron radiation techniques---are discussed alongside these data-driven machine learning (ML) frameworks. The scripts serve as illustrative workflow templates for contaminant detection, ecological risk quantification, and long-term predictive modelling; performance metrics are derived from simulated benchmark datasets and should be interpreted accordingly. Physics-Informed Neural Networks (PINNs) and ensemble learning are discussed as approaches outperforming classical statistical frameworks in contaminant transport prediction. A six-step multi-stage workflow links field sampling, ML-based analysis, and scenario-based risk modelling to soil ecosystem services (SES) assessment, supporting sustainable soil management and evidence-based environmental policy.}}
...
\textcolor{blue}{The study draws on more than 100 peer-reviewed publications identified through searches of Web of Science and Scopus using the terms ``soil pollution'', ``machine learning'', ``heavy metals'', ``PFAS'', ``microplastics'', and ``soil ecosystem services''. Sources were selected on the basis of methodological relevance, recency (predominantly 2015--2026), and coverage of the five contaminant groups addressed here. Table~\ref{tab1} summarises the principal detection methods and key references for each pollutant type. The Python workflows use simulated benchmark datasets that mirror the feature structures reported in the cited literature, enabling reproducible illustration of each computational pipeline.}

The comprehensive evaluation of the speciation, compartmentalization, and spatial distribution of major pollutants within soil organic complexes requires characterization of their intrinsic physicochemical properties. Soil contamination is primarily driven by hazardous substances from both anthropogenic activities---such as industrial effluents, agricultural chemical applications, and urban runoff---and geogenic processes, including weathering of parent bedrock and volcanic emissions, as schematically conceptualized in Figure~\ref{fig01}. 
...
\textcolor{blue}{Within the soil matrix, organic matter acts as a heterogeneous sorbent; humic substances, fulvic acids, and particulate organic matter possess functional groups---carboxyl, phenolic, and sulfhydryl moieties---that engage in complexation, ion exchange, and hydrophobic interactions with pollutants. The nature of these soil-pollutant complexes determines whether a contaminant remains sequestered, undergoes microbial degradation, or migrates into aquatic systems and trophic webs.} 
...
\textcolor{blue}{The cross-validated $R^2$ output from Script~\ref{lst:rf_hm} consistently exceeds 0.88 on the \emph{simulated} benchmark dataset, demonstrating that spectral bands and pH together account for the largest share of variance in predicted cadmium concentrations, consistent with published findings \cite{MCLAUGHLIN1999143}. Importantly, these metrics are derived from synthetic data generated to illustrate the computational procedure; they should not be interpreted as evidence of real-world predictive accuracy. The train/test split uses an 80/20 ratio with fixed random seeds; five-fold cross-validation is applied to estimate generalisation performance; and no external validation on field datasets was performed. When applied to real contaminated-site data, additional steps---including spatial cross-validation to address autocorrelation, hyperparameter tuning, and uncertainty quantification---would be required. These ML-driven pipelines nonetheless provide a reproducible template for monitoring pollutant kinetics across heterogeneous soil matrices.}
...
industrial and consumer products. PFAS are characterized by extreme chemical stability, environmental persistence, and high mobility across both aquatic and terrestrial matrices \cite{Brusseau}. Due to the strength of their carbon-fluorine bonds, these "forever chemicals" resist conventional biotic and abiotic degradation pathway mechanisms, culminating in chronic, long-term accumulation within soil profiles and contiguous aquifers. 
...
\textcolor{blue}{The LSTM model in Script~\ref{lst:lstm_pfas} captures superimposed seasonal oscillations and the long-term upward accumulation trend in PFAS concentrations, yielding a forecast RMSE below 0.05~ng/g on the simulated test horizon. This performance validates the utility of sequence models as early-warning tools for identifying sites approaching regulatory threshold exceedance \cite{Xu2023}. As with all workflows in this study, these metrics are derived from synthetic data and indicate the potential of the approach rather than confirmed field accuracy.}
...
that traditional statistical models fail to resolve \cite{2939785}. For instance, Random
...
including artificial neural networks \cite{Anifowose2024}, random
forest algorithms \cite{Shaheen2018,Foronda2023}, support vector machines
\cite{Gholizadeh2020}, and deep learning architectures
\cite{Hu2024}, are increasingly applied to identify complex
nonlinear relationships among soil physicochemical parameters
\cite{Aljanabi2025}, contaminant distributions
\cite{Shaheen2018}, and environmental variables
\cite{Gholizadeh2020}.
...
The seven Python workflows presented in Section~\ref{sec2} collectively demonstrate a reproducible computational pipeline covering the full analytical arc---from raw spectroscopic and field data ingestion to spatial risk mapping and regulatory index computation. Table~\ref{tab:script_results} summarises the principal performance metrics yielded by each script on the respective simulated benchmark datasets. \textcolor{blue}{All reported metrics (R$^2$, ROC-AUC, accuracy, RMSE) were obtained from simulated datasets generated to illustrate the computational procedure; they represent indicative baselines and should not be interpreted as evidence of real-world predictive accuracy or transferability to field conditions. Each workflow employs an 80/20 (or 75/25) train/test split with stratification where appropriate, and five-fold cross-validation for regression tasks. No external validation on independent field datasets was performed. For geospatial tasks (Script~7), spatial autocorrelation and potential data leakage between training and test points should be addressed when adapting these workflows to real contaminated-site datasets. These results serve as templates for adaptation to real field data, where additional validation steps would be required.}
...
data \cite{Guan}. The XGBoost classifier (Script~2) achieved
